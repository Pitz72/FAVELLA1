# FAVELLA 1 — Specifica formale della grammatica (v1.4.0)
...
> Versione corrente: **1.4.0** (grammatica e motore).
>
> **Novità v1.4.0 — i pulsanti-verbo e l'architettura (L-7).** Una sola frase
> nuova, additiva: **`def_comandi`** (`I comandi si scrivono.` / `I comandi si
> scelgono con i pulsanti.` / `I comandi si scrivono oppure si scelgono con i
> pulsanti.`) dice come il giocatore dà i comandi nelle pagine che mostrano i
> pulsanti-verbo (pagina esportata, sito); senza la frase valgono entrambi i
> modi. Il resto non tocca la grammatica: il motore non scrive più con `print()`
> ma emette un **flusso di eventi** tipizzati; `gioco.pulsanti()` dice quali
> verbi, oggetti, uscite e risposte proporre; `compilatore.py` è diviso in
> nucleo, `strumenti_ide.py` ed `esportazione.py`. Nessuna frase valida in 1.3
> cambia significato e il testo che il motore scrive è identico. Keyword
> aggiunte: `comandi`, `scrivono`, `scelgono`, `pulsanti` (non riservate). Vedi §23.
>
...
> anche un verbo dichiarato dall'autore, e fra virgolette un comando di più parole
> (`"lancia il cibo" è come "getta il cibo".`). Il resto è runtime e strumenti:
> i comandi di servizio **SALVA** e **CARICA** (§20), il collaudo dinamico
> (`favella1 esplora`, `favella1 collaudo --finali`) e, nel collaudo statico,
> l'avviso sulle scorte che non crescono mai. Nessuna frase valida in 1.1 cambia
> significato. Keyword aggiunte: nessuna. Vedi §20.
>
> **Novità v1.1.0 — il posto iniziale degli oggetti.** Prima estensione dopo la
> milestone, piccola e additiva: nessuna frase valida in 1.0 cambia significato.
> Nuova regola **`def_posto`** (`"Il" "posto" _PREP_DESCR ENTITA "è" TESTO_QUOTATO "."`):
> `Il posto della mappa è "Su un mobile, una MAPPA piegata della provincia.".`
> Risolve un difetto di scrittura ricorrente: l'autore che descrive un oggetto
> dentro la prosa della stanza lo vede «restare descritto» anche dopo che il
> giocatore l'ha preso. Keyword aggiunta: `posto`. Vedi §18.
> La 1.1.0 corregge anche la semantica documentata del demone `Quando` (§6) e
> fissa il comportamento osservato scrivendo Il Viaggiatore (§19), con un avviso
> nuovo sulla capienza dell'inventario.
>
...
```ebnf
    ...
    ?dichiarazione: def_stanza
                  ...
                  | def_illumina
                  | def_sinonimo
                  | def_di_scena          // [1.3.0 / M-8]
                  | def_anche_in          // [1.3.0 / M-8]
    ...
    // [1.1.0] Posto iniziale di un oggetto. Inizia con "Il" come def_giocatore e
    // def_dialogo_inizio: il lookahead "posto" vs "giocatore"/"dialogo" decide.
    ...
    // a priorità più alta), PREP_LUOGO e "una"/"un"/"uno" → LALR(1) 0-ambiguo.
    ...
    // --- [1.3.0 / M-10] PERSONAGGI CHE TENGONO OGGETTI, ARGOMENTI ---
    // Dopo ENTITA il lookahead "ha" è disgiunto da è/sono/si/collega/dà/illumina/al.
    def_png_ha: ENTITA "ha" ENTITA "."
    // Inizia con "Se" maiuscolo: nessun'altra dichiarazione comincia così.
    ...
    // --- [1.4.0] PULSANTI-VERBO: come il giocatore dà i comandi ---
    ...
    ?cond_base: cond_possesso
              ...
              | cond_contatore_lte
              | cond_probabilita
              | cond_posizione_oggetto        // [1.3.0 / G-6]
              | cond_posizione_oggetto_neg
              | cond_png_ha                   // [1.3.0 / M-10]
              | cond_png_ha_neg
              | cond_non_gruppo
              | "(" cond_or ")"
    ...
    // [1.3.0 / G-6] Dove stanno oggetti e personaggi. Dopo 'ENTITA è' il lookahead
    // PREP_LUOGO è disgiunto da PROPRIETA → 0-ambiguo. 'se il gatto è qui' è
    // cond_proprieta con la proprietà speciale 'qui' (vedi §22): come parola
    // chiave, 'qui' vincerebbe su ogni proprietà nello stato dopo la copula.
    cond_posizione_oggetto: ENTITA _copula PREP_LUOGO ENTITA
    cond_posizione_oggetto_neg: ENTITA "non" _copula PREP_LUOGO ENTITA
    // [1.3.0 / M-10] Dopo ENTITA il lookahead "ha" / "non" "ha" è disgiunto da _copula.
...
- **`demone_quando` — `Quando [cond] (diventa vera)?: …`** (sul **fronte di
  salita**): scatta nel turno in cui la condizione passa da falsa a vera, e
  **ogni volta** che lo rifà. Se la condizione torna falsa e poi di nuovo vera
...
## 18. Il posto iniziale degli oggetti v1.1.0

È l'equivalente dell'*initial appearance* di Inform 7, ridotto all'osso.

- **Sintassi.** `Il posto della mappa è "Su un mobile, una MAPPA piegata della provincia.".`
  La preposizione articolata è la stessa di `def_descrizione` (`_PREP_DESCR`); il
  testo è una stringa singola, con i segnaposto `[nome]` come ogni testo d'autore.
  Nessuna clausola `se` e nessuna variante: il posto descrive *come si trova*
  l'oggetto all'inizio, non uno stato che cambia.

- **Semantica.** Il transformer registra la frase in `Oggetto.posto` (differita a
  `valida_post`, quindi l'ordine delle frasi non conta). A runtime `mostra_stanza`
  stampa, dopo la descrizione della stanza, i posti di tutti gli oggetti della
  stanza per cui `Oggetto.al_suo_posto()` è vero; quegli oggetti
  **non** compaiono in «Puoi vedere qui». Le frasi sono raccolte in **un solo
  capoverso**, nell'ordine di collocazione: per questo ciascuna va scritta
  autonoma («Sul sedile, un DIARIO…», non «Accanto, un DIARIO…»), perché deve
  reggere anche quando le altre sono sparite. `al_suo_posto()` vale finché
  `Oggetto.spostato` è falso: il segno diventa vero alla prima chiamata di
  `Mondo.rimuovi_da_posizione` (prendere, una conseguenza di spostamento, il
  movimento di un personaggio) e non torna più falso. Un oggetto preso e poi
  lasciato, anche nello stesso punto, ricompare nell'elenco generico: la frase
  d'autore non descriverebbe più il vero stato delle cose.

- **Validazione.** Posto per un'entità inesistente o per una **stanza**: errore
  bloccante. Più posti per lo stesso oggetto: avviso, vale l'ultimo. Posto per un
  oggetto che non comincia *direttamente* in una stanza (in un contenitore, su un
  supporto, in inventario, nel nulla): avviso, la frase non sarebbe mai mostrata.
  Il testo del posto passa per il controllo dei segnaposto come le descrizioni.
  Dopo l'inventario iniziale il segno `spostato` viene azzerato per tutti gli
  oggetti: conta solo ciò che accade in gioco.

- **ANNULLA-safe.** `posto` e `spostato` sono attributi dell'oggetto, già compresi
  nelle istantanee pre-turno: annullare una presa riporta la frase in scena.
...
Nessuna regola di grammatica nuova. Scrivere *Il Viaggiatore* (39 luoghi, 13
personaggi, 6 finali) ha messo alla prova il motore su una storia lunga; qui si
fissa ciò che è emerso, così che la spec descriva il motore com'è.

- **`Quando` riscatta a ogni fronte.** Vedi §6: il demone scatta ogni volta che
  la condizione passa da falsa a vera, non una volta per partita. Un evento una
  tantum si guarda con uno stato.

- **Precedenza delle regole `Invece di`.** Una regola con bersaglio
  (`Invece di getta il cibo: …`) vince sempre su quella generica sullo stesso
  verbo (`Invece di getta: …`), in qualunque ordine siano scritte. Fra regole
  senza bersaglio vince la prima dichiarata.

- **Il parser viene prima delle regole.** Se il giocatore nomina un oggetto che
  non è raggiungibile, il motore risponde «Non vedo…» senza valutare le regole:
  una regola su un oggetto lontano non scatta mai.

- **`"x" è come <verbo>` vale per i verbi della libreria.** Per un verbo
  dichiarato dall'autore (`"getta" è un comando.`) il sinonimo non ne eredita le
  regole: ogni variante va dichiarata come comando a sé (`"lancia il cibo"`).

- **`è in inventario` e la capienza.** La conseguenza `e adesso X è in inventario`
  non controlla `Il giocatore può portare N oggetti` (lo fa solo il prendere del
  giocatore): l'inventario può superare la capienza. Il comportamento resta
  quello della 1.0; dalla 1.1.0 il compilatore lo segnala con un **avviso**, una
  volta per oggetto, quando la storia dichiara una capienza.
  Test: `test_capienza_avviso_su_spostamento_in_inventario`.

- **Descrizioni e battute con `se`.** Vince la prima variante la cui condizione è
  vera, nell'ordine di scrittura; quella senza condizione fa da ripiego.
...
- **ANCORA e ANNULLA.** Fino alla 1.2.0 `Mondo.ultimo_comando` era stato *di
  ...
  Test: `test_ancora_dopo_annulla_ripete_il_comando_disfatto`.

- **Determinismo.** Tutto il caso passa da `Mondo.rng`, con seme fisso alla
  compilazione (`SEME_CASUALE_DEFAULT`): la stessa sequenza di comandi riproduce
  lo stesso stato, anche fra sistemi operativi diversi.

- **Limite del collaudo statico.** `favella1 collaudo` non modella le scorte
  (contatori che salgono e scendono, come acqua o cibo): un vicolo cieco per
  risorse esaurite non viene visto.

## 20. Salvare, collaudare giocando, sinonimi per ogni verbo v1.2.0

Le quattro proposte nate da *Il Viaggiatore*, in un'unica versione. Una sola tocca
la grammatica.

- **Sinonimi dei verbi d'autore (grammatica).** `def_sinonimo` ha una seconda
  forma, `TESTO_QUOTATO "è" "come" TESTO_QUOTATO "."`. Il bersaglio può essere un
  verbo della libreria (come prima), un comando dichiarato dall'autore
  (`"lancia" è come getta.`) o, fra virgolette, un comando di più parole
  (`"butta via il cibo" è come "getta il cibo".`). Se il bersaglio non è un verbo
  di libreria la decisione è differita a `valida_post` (`_pending_sinonimi`),
  così il comando può essere dichiarato anche dopo il sinonimo; se non esiste
  nemmeno lì resta l'avviso di sempre. A runtime i sinonimi di più parole
  partecipano al riconoscimento per prefisso più lungo
  (`_match_verbo_multiparola`), poi il sinonimo è riscritto nel suo bersaglio
  prima di ogni altro trattamento: regole `Invece di`, anafora e logica di
  default si comportano come per il verbo originale.

- **SALVA / CARICA (runtime).** `salva [nome]` e `carica [nome]` (anche
  `salvare`, `caricare`, `ripristina`; nome predefinito `partita`) sono comandi
  di servizio come ANNULLA: non consumano un turno, non entrano nella sequenza
  salvata e valgono anche durante una conversazione. Non vengono intercettati se
  l'autore ha dichiarato quel verbo (`"carica" è un comando.` per un fucile).
  - *Cosa si salva.* Non il mondo (sarebbe `pickle`, fragile fra versioni e
    pericoloso con file altrui) ma la **sequenza effettiva dei comandi**
    (`Mondo._registro_comandi`): i turni disfatti con ANNULLA ne escono (la pila
    `_pos_registro`, allineata a `_storia_stati`, ricorda dove tagliare; una
    conversazione è un solo passo, come per ANNULLA), ANCORA vi entra col comando
    che ripete. In più: `ultimo_comando`, il turno, l'impronta della storia e
    l'**impronta SHA-256 dello stato** (`Mondo.impronta_stato()`: luogo, turno,
    esito, dialogo, inventario, variabili, oggetti con posizione, proprietà,
    `spostato` e contenuto, stanze buie, `era_vera` dei demoni, stato di `rng`).
    Il file è JSON (`formato: "favella-salvataggio"`, `versione: 1`).
  - *Come si carica.* `imposta_posizione_iniziale` fotografa il mondo a partita
    non cominciata (`_stato_iniziale`, `_impronta_iniziale`). CARICA controlla
    che il salvataggio sia di questa storia (impronta iniziale), riparte da
    quella fotografia e rigioca la sequenza: la testa senza istantanee
    (`_senza_istantanee`, niente copia profonda a ogni turno), gli ultimi
    `CODA_ANNULLA` = 40 comandi con le istantanee accese, così ANNULLA funziona
    anche subito dopo. La coda comincia sempre fuori da una conversazione. Poi
    ripristina `ultimo_comando` e confronta l'impronta: se differisce (storia
    cambiata dopo il salvataggio) lo dice. Se il rigioco si interrompe, la
    partita in corso resta com'era.
  - *Dove.* `mondo.archivio_salvataggi` (un oggetto con `scrivi(nome, testo)` e
    `leggi(nome)`) se l'host ne fornisce uno; altrimenti file `<nome>.salvataggio`
    nella cartella di lavoro (terminale, playground locale) oppure, sotto Pyodide,
    il `localStorage` del browser con la chiave `favella-salvataggio:<impronta>:<nome>`
    (sito, esportazione HTML).
  - *Determinismo.* Il metodo regge perché il motore è deterministico (§19): il
    caso passa da `Mondo.rng`, seme fisso. Collaudo: 12 partite del Viaggiatore
    fino a 128 comandi, con ANNULLA e ANCORA, ricaricate in un'istanza nuova:
    impronta identica e risposte identiche nei comandi successivi.

- **Collaudo dinamico (strumento).** Nuovo modulo `esploratore.py`, fuori dal
  runtime. `favella1 esplora storia.fav` gioca partite a caso con quattro
  «caratteri» (curioso, maldestro, chiacchierone, frettoloso) che esplorano,
  sbagliano a scrivere, chiedono cose che non ci sono, annullano. Segnala
  ECCEZIONE, ERRORE, SEGNAPOSTO, INVISIBILE, USCITA (gravi: codice d'uscita 1),
  NEGATIVO, CAPIENZA, SILENZIO e gli stalli, con i comandi per riprodurre ogni
  anomalia; misura la copertura di luoghi, nodi di dialogo e oggetti.
  `favella1 collaudo storia.fav --finali` elenca le conseguenze di fine partita
  dichiarate e dice quali sono state raggiunte e da quale partita. `--percorso
  FILE` (ripetibile) gioca per intero una sequenza scritta a mano e fa partire
  l'esploratore anche da ogni stanza nuova che tocca. Stesso seme, stesse partite.

- **Scorte che non crescono (collaudo statico).** La risalita della catena della
  vittoria ora tiene conto del **verso**: «è almeno N» lo produce solo chi fa
  crescere il contatore (`aumenta`, `diventa`), «è meno di N» solo chi lo fa
  calare. Nuova sezione del report, `scorte`: una condizione di regola, demone o
...
## 23. I pulsanti-verbo e l'architettura del motore (1.4.0)

La 1.4.0 chiude l'ultima parte di L-7 (`analisi-critica-1.2.1.md`) e realizza
l'idea dei pulsanti-verbo (§8.4 dell'analisi). Una frase nuova; il resto è
motore e strumenti. Le frasi valide in 1.3 restano valide e significano la
stessa cosa; il testo che il motore scrive è identico, byte per byte.

### 23.1 La frase `I comandi si …`

```
I comandi si scrivono.                                   → solo il campo di testo
I comandi si scelgono con i pulsanti.                    → solo i pulsanti
I comandi si scrivono oppure si scelgono con i pulsanti. → entrambi (predefinito)
```

- Vale nelle pagine che mostrano i pulsanti (la pagina esportata con
  `favella1 esporta`, le cassette-gioco del sito). Il terminale resta a testo.
- Senza la frase valgono **entrambi** i modi: il giocatore può scrivere e può
  toccare, e nascondere i pulsanti se non li vuole.
- Due frasi con modi diversi: vale l'ultima, con un avviso.
- Si distingue dalle altre dichiarazioni sul primo token `I` e, dopo `si`, sul
  lookahead `scrivono`/`scelgono`; dopo `scrivono`, `.` o `oppure`. Un nome che
  comincia con «I comandi» (un oggetto `I comandi della nave`) vince per
  lunghezza, come per `Le uscite nominano…` (§22.5). LALR(1) 0-ambiguo (guardia
  nella suite). Le parole nuove non sono riservate.
- `Mondo.modo_comandi` ∈ {`entrambi`, `pulsanti`, `testo`}; dato statico (fuori
  dalle istantanee).

### 23.2 Che cosa propongono i pulsanti (`gioco.pulsanti`)

`pulsanti(mondo)` restituisce, in JSON, ciò che un'interfaccia può proporre
adesso: `modo`, `fase`, `scelte`, `oggetti`, `verbi`, `uscite`, `servizio`. Il
comando si compone come `verbo + " " + primo.testo [+ " " + secondo.testo]`: le
frasi arrivano già scritte in italiano (`alla guardia`, `nella cassa`, `sul
tavolo`) e passano da `elabora_comando` come se il giocatore le avesse scritte.
Regole, turni, ANNULLA e salvataggi non cambiano.

I criteri:

1. **Si filtra solo con ciò che il giocatore sa già**: che cosa porta, che
   cosa vede, chi è un personaggio. Mai con ciò che scoprirebbe provando:
   «Prendi» propone anche ciò che non si lascia prendere (non le persone, a
   meno che una regola ne parli); «Lascia», «Dai» e «Mostra» solo ciò che si
   porta; «Dai» e «Mostra» a un personaggio (o a ciò che una regola nomina come
   secondo oggetto); «Metti» dentro i contenitori e sopra i supporti (`sul
   tavolo`), mai dentro un personaggio. Un primo oggetto compare solo se esiste
   un secondo diverso da lui, quando il secondo è obbligatorio.
2. **Ciò che l'autore ha inventato resta suo.** I verbi d'autore di parole
   nuove e gli argomenti di conversazione, con il campo di testo accanto,
   compaiono solo dopo che il giocatore li ha trovati scrivendo
   (`Mondo.verbi_scoperti`, `Mondo.argomenti_scoperti`: memoria del giocatore,
   che ANNULLA non cancella). Con i soli pulsanti compaiono da subito: non c'è
   altro modo di usarli. Un verbo d'autore che usa una parola della libreria
   (`"accendi" è un comando.`) non è un segreto: ha il suo pulsante, al posto
   di quello della libreria.
3. **Verbi della libreria**: `esamina`, `prendi`, `lascia`, `usa` sempre; gli
   altri quando la storia ne dà un motivo (oggetti apribili o chiusi,
   accendibili o luminosi, commestibili, bevibili; contenitori e supporti) o
   una regola scritta per quel verbo, con le sue preposizioni. `Parla con`
   se la storia ha dialoghi; `Chiedi a` con gli argomenti la cui condizione è
   vera ora (valutata senza consumare il caso).
4. **Fasi**: in un dialogo le opzioni (e «Chiudi la conversazione»), davanti a
   una domanda di conferma «Sì»/«No», davanti a un nome ambiguo i candidati, a
   partita finita «Annulla», «Ricomincia», «Carica». Al buio si vede solo ciò
   che si porta, e le uscite solo con i soli pulsanti (senza il nome della
   stanza); con i soli pulsanti compaiono anche le direzioni che una regola
   `vai` rende percorribili qui e ora.
...
equivalente che porta allo stesso identico stato (impronta SHA-256). Restano
fuori solo i comandi a più oggetti (`prendi tutto`, `prendi la chiave e la
torcia`), che con i pulsanti si fanno un oggetto alla volta. Ogni frase
composta con i pulsanti è capita dal parser (un test della suite lo verifica su
tre storie, in entrambi i modi).

### 23.3 L'uscita del motore come flusso di eventi (L-7)

Il motore non scrive più con `print()`: ogni cosa che dice al giocatore è un
`Evento(tipo, testo, stacco, dati)` consegnato a `mondo.uscita`
(`favella_utils.scrivi`). Tipi: `intestazione`, `stanza` (dati: `id`), `testo`,
`elenco`, `domanda`, `dialogo`, `opzione` (dati: `numero`, `scelta`),
`sistema`, `fine` (dati: `esito`), `errore`. `stacco` è la riga vuota prima.

- Senza un'uscita dell'host (`mondo.uscita is None`) gli eventi vanno sul
  flusso standard corrente, esattamente come prima: CLI, TRASCRIZIONE e test che
  leggono stdout non cambiano.
- Gli host raccolgono gli eventi con `raccogli_uscita(mondo)`; `testo()` li
  rimette in fila come il terminale. IDE (sidecar: campo `events`), playground,
  esploratore, pagina esportata e sito non dirottano più stdout.
- CARICA rigioca la partita con un'`UscitaMuta`.
- `mondo.uscita` è fuori dalle istantanee e dall'impronta.

### 23.4 Il compilatore diviso (L-7)

`compilatore.py` è il nucleo: grammatica, transformer, validazione, `Includi`,
`analizza_file`, `analizza_file_strutturato`, `compila_mondo`. Gli strumenti
stanno in `strumenti_ide.py` (analisi per gli editor visuali, riordino,
serializzatore) ed `esportazione.py` (la pagina HTML). I vecchi import
(`from compilatore import esporta_html`) funzionano ancora (import pigro). Il
