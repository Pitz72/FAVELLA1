# FAVELLA 1 — Specifica formale della grammatica (v0.22.0)
...
>
> **Nota sulle release 0.20.0 e 0.21.0.** Sono state **solo runtime** (parser dei
> comandi del giocatore), senza toccare la grammatica del sorgente: A1 pronomi e
> anafora (`prendila`, `prendi quella`) e A3 comandi di servizio (ANNULLA,
> ANCORA, TRASCRIZIONE). Non compaiono in questa EBNF perché non sono sintassi
> d'autore.
>
> **Novità v0.22.0 — «A2: varietà nelle risposte» (UNICA modifica di grammatica
> dalla 0.19.0):** una descrizione può avere **più varianti**, con due politiche.
> `La descrizione del mare è una di: "…", "…".` (casuale, mai due volte di fila la
> stessa) e `La descrizione del faro è in sequenza: "…", "…".` (rotazione, l'ultima
> resta). La regola `def_descrizione` delega il valore a `descr_valore`
> (`descr_singola` | `descr_casuale` | `descr_sequenza`). La casualità usa un
> generatore del mondo con **seme fisso** (riproducibile; lo stato è catturato da
> ANNULLA). Vedi §9.
>
...
- **A7 (verbi intransitivi).** `dichiara_verbo(verbo, intransitivo=True)` popola
  ...
  `_personalizzata` (`richiede_oggetto=True`, storica), gli **intransitivi** a
  `_personalizzata_intransitiva` (`richiede_oggetto=False`). A runtime
  (`gioco._esegui_comando`) un verbo intransitivo salta il controllo dell'oggetto
  e raggiunge la **fase delle regole globali** (la stessa di `guarda`), dove una
  `Invece di accelera: …` senza bersaglio lo gestisce (con condizioni e
  conseguenze, incluso `vinci`/`perdi`). Vale anche multi-parola.
- **A8 (inventario iniziale).** `def_giocatore_inventario` registra l'oggetto in
  `_pending_inventario_iniziale`; in `valida_post` (dopo le posizioni) l'oggetto
  è rimosso dall'eventuale collocazione e messo in `Mondo.inventario`
  (`posizione = "inventario"`). Oggetto inesistente = errore bloccante. Un
  oggetto per frase; più oggetti = più frasi.
- **A9 (esito temporale senza testo).** La regola inline `_esito_temporale`
  ...
  almeno una conseguenza. `_crea_evento`/`_crea_demone` estraggono la risposta
  **per tipo** (l'unica `str`; assente → `""`). Il runtime stampa la battuta solo
  se non vuota: un «tick» come `Ogni 2 turni: aumenta la stanchezza.` muta lo
  stato in silenzio.

Riservate aggiunte da questo ciclo: `senza`, `oggetto` (A7). `ha` (A8) era già
riservata (`cond_possesso`).

## 9. Varietà nelle risposte — semantica v0.22.0 (A2)

`def_descrizione` delega il valore a **`descr_valore`**, che ha tre forme alias:

- **`descr_singola`** (`"…"`): una stringa, comportamento storico. Il transformer
...
Il valore-descrizione (stringa o `VariantiDescrizione`) è memorizzato come prima,
sia come descrizione di base sia come variante condizionale (`se …`). A
render-time, `strutture._descrizione_attuale` risolve una `VariantiDescrizione`
chiamando `.scegli(mondo)`:

- **sequenza**: avanza un indice interno a ogni richiesta; sull'ultima resta;
- **casuale**: pesca da `mondo.rng` (`random.Random(SEME_CASUALE_DEFAULT)`)
  evitando di ripetere subito l'ultima variante mostrata.

`mondo.rng` e gli indici delle `VariantiDescrizione` sono **stato del mondo**:
catturati e ripristinati dalle istantanee di **ANNULLA** (l'undo riavvolge anche
la casualità → riproducibilità totale, utile al futuro giocatore-robot B1).

La validazione dei segnaposto `[var]` e il linter ispezionano **ogni** variante
(`testi_di_descrizione`); la serializzazione per l'IDE usa una variante
rappresentativa (`descrizione_display`). Riservata aggiunta: `sequenza` (`una`,
`di`, `in` erano già riservate). Grammatica **LALR(1) 0-ambigua** per costruzione.
