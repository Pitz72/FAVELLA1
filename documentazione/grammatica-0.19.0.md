# FAVELLA 1 — Specifica formale della grammatica (v0.19.0)
...
>
> **Novità v0.19.0 — «Lezioni dallo stress-test di genere»** (tre integrazioni
> emerse costruendo un genere non previsto dal design — il simulatore di guida
> testuale; vedi §8):
> - **A7** **Verbi intransitivi**: `"accelera" è un comando senza oggetto.`
>   dichiara un comando che il giocatore digita da solo, gestito da una regola
>   GLOBALE `Invece di accelera: …`. La dichiarazione si sdoppia in
>   `verbo_con_oggetto` (storico) e `verbo_senza_oggetto`.
> - **A8** **Inventario iniziale del giocatore**: `Il giocatore ha la torcia.`
>   (`def_giocatore_inventario`). Un oggetto per frase.
> - **A9** **Esito temporale senza testo** («tick» silenzioso): la battuta
>   `dire "…"` di eventi e demoni è ora **opzionale**, purché ci sia almeno una
>   conseguenza (`Ogni 3 turni: diminuisci il carburante.`). Regola inline
>   condivisa `_esito_temporale`.
