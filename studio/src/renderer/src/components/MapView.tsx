...
// posizione salvata; trascinare un nodo ne fissa la posizione anche fra i reload.
...
export default function MapView({ compact = false, editable = false }: MapViewProps): JSX.Element {
  ...
  // [UX T5] Nel dock la mappa è SEMPRE editabile (come gli altri editor del mondo):
  // niente più interruttore «✏️ Modifica». Resta in sola lettura solo nella finestra
  // di gioco, dove `editable` è false.
  const attivo = editable

  // Da editabile serve l'outline (per gli span delle frasi da eliminare).
  useEffect(() => {
  ...
  const edges = useMemo(() => (graph ? costruisciArchi(graph) : ([] as Edge[])), [graph])

  // Nodi in stato locale (react-flow li muove durante il drag via onNodesChange).
  // Posizione = quella SALVATA manualmente se presente, altrimenti layout a griglia.
  ...
  useEffect(() => {
    ...
    const saved = loadPositions(activePath)
    ...
    setNodes(next)
  }, [graph, current, activePath, setNodes])

  const onNodeDragStop = (_e: React.MouseEvent, node: Node): void => {
    savePosition(activePath, node.id, node.position)
  }
  const riallinea = (): void => {
    clearPositions(activePath)
    if (!graph) return
    const auto = disponiStanze(graph)
    setNodes((ns) =>
      ns.map((n) => {
      ...
      {editable && (
        <div className="map-toolbar">
          ...
          <button
            ...
            onClick={riallinea}
            title="Ridisponi automaticamente le stanze (azzera le posizioni manuali)"
          >
            ⤢ Riallinea
          </button>
