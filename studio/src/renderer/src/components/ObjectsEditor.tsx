...
export default function ObjectsEditor(): JSX.Element {
  ...
  // [Livello 7] Capacità di trasporto BASE del giocatore. 0/null → illimitata
  // (rimuove la frase «può portare N oggetti»).
  ...
  // [Livello 7] Bonus di capacità dell'oggetto selezionato («dà N spazi»). 0 → rimuove.
...
// [Livello 7] Capacità di trasporto BASE del giocatore: «Il giocatore può portare
// N oggetti.» (vuoto = illimitata). Controllo globale in cima al pannello Oggetti.
function CarryBaseField({
  value,
  onSet
}: {
  value: number | null
  ...
  return (
    <div className="objed-carry">
      <span className="var-note">🎒 Il giocatore può portare</span>
      <input
        type="number"
        min={1}
        placeholder="∞"
        ...
        onBlur={salva}
        title="Quanti oggetti può tenere in inventario (vuoto = illimitato)"
      />
      <span className="var-note">oggetti (vuoto = illimitato)</span>
    </div>
  )
}

// [Livello 7] Bonus di capacità dell'oggetto: «X dà N spazi.» (0 = nessuno).
function CarryBonusField({
  value,
  onSet
}: {
  value: number
  ...
  return (
    <div className="objed-field objed-carry">
      <label>Capacità (zaino)</label>
      <input
        type="number"
        min={0}
        value={v}
        onChange={(e) => setV(e.target.value)}
        onBlur={salva}
        title="Spazi di trasporto in più mentre l'oggetto è nell'inventario (0 = nessuno)"
      />
      <span className="var-note">dà N spazi in più (0 = nessuno)</span>
    </div>
  )
}
