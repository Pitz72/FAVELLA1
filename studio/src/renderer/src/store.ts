...
interface StudioState {
  ...
  // [Autoformat] Riordina il file attivo (sostituisce l'intero buffer, undo nativo).
  reorderActive: () => Promise<void>
  // [Fase 7] Esporta la storia attiva come HTML autoportante (dialogo di salvataggio).
...
export const useStudio = create<StudioState>((set, get) => ({
  ...
  loadDialogues: async () => {
    ...
    try {
      const d = await window.favella.worldDialogues(activePath, file?.content)
  ...
  mapAddConnection: async (fromId, direction, toId, opposite) => {
    ...
    if (!outline.directions.includes(dir)) {
      ...
      try {
        ...
        set({ editError: 'Errore di serializzazione: ' + messaggioErrore(e) })
        return
      }
    }
    ...
    if (!res.ok || !res.text) {
      set({ editError: res.error ?? 'Impossibile generare la connessione.' })
      return
    }
  ...
  mapAddRoom: async (name) => {
    ...
    if (!res.ok || !res.text) {
      set({ editError: res.error ?? 'Impossibile generare la stanza.' })
      return
    }
    const file = openFiles.find((f) => f.path === activePath)
    if (!file) return
  ...
  applyStatement: async (spec, span) => {
    ...
    if (!res.ok || !res.text) {
      set({ editError: res.error ?? 'Operazione non riuscita.' })
      return
    }
    let edit: BufferEdit
    if (span) {
  ...
  deleteStatement: async (span) => {
    ...
    if (!activePath) return
    if (!stessoPercorso(span.file, activePath)) {
      set({ editError: 'Questa frase è in un altro file (incluso): aprilo per modificarla da qui.' })
      return
    }
    const file = openFiles.find((f) => f.path === activePath)
    if (!file) return
  ...
  // (una riga per frase). Usato dall'editor dialoghi per creare un nodo intero
  // (battuta + eventuale «è un personaggio» + nodo d'ingresso) in un solo edit:
  // così non ci sono span che slittano fra una frase e l'altra.
  appendStatements: async (specs) => {
    set({ editError: null })
    const { activePath, openFiles } = get()
    if (!activePath || specs.length === 0) return
    const texts: string[] = []
    ...
    const file = openFiles.find((f) => f.path === activePath)
    if (!file) return
    const edit: BufferEdit = { kind: 'append', text: texts.join('\n') }
    const nuovo = applicaBufferEdit(file.content, edit)
    set((s) => ({
      openFiles: s.openFiles.map((f) => (f.path === activePath ? { ...f, content: nuovo } : f)),
      pendingEdit: { path: activePath, edits: [edit], nonce: (s.pendingEdit?.nonce ?? 0) + 1 }
  ...
  reorderActive: async () => {
    const { activePath, openFiles } = get()
    if (!activePath?.toLowerCase().endsWith('.fav')) return
    const file = openFiles.find((f) => f.path === activePath)
    if (!file) return
    let res
    try {
      res = await window.favella.reorderSource(activePath, file.content)
    } catch (e) {
    ...
    const nuovo = res.text.replace(/\n$/, '')
    if (nuovo === file.content) {
      set({ gameNotice: 'Il file è già ordinato.' })
      return
    }
    const lineCount = file.content.split('\n').length
    const edit: BufferEdit = { kind: 'replaceLines', startLine: 1, endLine: lineCount, text: nuovo }
    set((s) => ({
      openFiles: s.openFiles.map((f) => (f.path === activePath ? { ...f, content: nuovo } : f)),
      pendingEdit: { path: activePath, edits: [edit], nonce: (s.pendingEdit?.nonce ?? 0) + 1 },
      gameNotice: 'Sorgente riordinato.'
    }))
    await get().loadOutline()
    await get().loadWorldGraph()
  },
  ...
  exportGame: async () => {
    ...
    let res
    try {
      res = await window.favella.exportGameHtml(activePath, file?.content)
    } catch (e) {
  ...
  renameDialogueNode: async (oldLabel, newLabel) => {
    ...
    if (!d?.ok || !activePath || !nl || nl === oldLabel) return
    ...
    if (jobs.some((j) => !stessoPercorso(j.span.file, activePath))) {
      set({ editError: 'Alcune frasi del nodo sono in un altro file: rinomina dal testo.' })
      return
    }
    // Serializza tutte le frasi nuove (ogni frase è su UNA riga → i replace non slittano).
    const edits: BufferEdit[] = []
    for (const j of jobs) {
      let res
      try {
        res = await window.favella.serializeStatement(j.spec)
      } catch (e) {
        set({ editError: 'Errore di serializzazione: ' + messaggioErrore(e) })
        return
      }
      if (!res.ok || !res.text) {
        set({ editError: res.error ?? 'Rinomina non riuscita (opzione troppo complessa: usa il testo).' })
        return
      }
      edits.push({ kind: 'replaceLines', startLine: j.span.line, endLine: j.span.endLine, text: res.text })
    }
    const file = openFiles.find((f) => f.path === activePath)
    if (!file) return
    let content = file.content
    for (const e of edits) content = applicaBufferEdit(content, e) // single-line: ordine ininfluente
    set((s) => ({
      openFiles: s.openFiles.map((f) => (f.path === activePath ? { ...f, content } : f)),
      pendingEdit: { path: activePath, edits, nonce: (s.pendingEdit?.nonce ?? 0) + 1 }
    }))
    await get().loadOutline()
    await get().loadWorldGraph()
    await get().loadRules()
  ...
  deleteDialogueNode: async (label) => {
    ...
    if (spans.some((s) => !stessoPercorso(s.file, activePath))) {
      set({ editError: 'Alcune frasi del nodo sono in un altro file: eliminale dal testo.' })
      return
    }
    // Elimina dal BASSO verso l'ALTO così le righe sopra non slittano.
    const ordinati = [...spans].sort((a, b) => b.line - a.line)
    const edits: BufferEdit[] = ordinati.map((s) => ({
      kind: 'deleteLines',
      startLine: s.line,
      endLine: s.endLine
    }))
    ...
    if (!file) return
    let content = file.content
    for (const e of edits) content = applicaBufferEdit(content, e)
    set((s) => ({
      openFiles: s.openFiles.map((f) => (f.path === activePath ? { ...f, content } : f)),
      pendingEdit: { path: activePath, edits, nonce: (s.pendingEdit?.nonce ?? 0) + 1 }
    }))
