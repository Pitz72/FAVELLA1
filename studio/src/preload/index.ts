...
const api = {
  ...
  /** Ultime righe di stderr del motore (traceback Python), per la diagnostica. */
  sidecarLastError(): Promise<string> {
    return ipcRenderer.invoke('sidecar:lastError')
  },
  /**
  ...
  /** [Autoformat] Riordino canonico del sorgente di un file singolo. */
  ...
  writeExport(html: string, name: string): Promise<{ ok: boolean; path?: string }> {
    return ipcRenderer.invoke('game:writeExport', { html, name })
  },
